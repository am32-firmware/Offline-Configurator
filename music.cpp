...
static const struct {
  ...
  const char *sharp;
    ...
    {"DB", "C#"}, {"EB", "D#"}, {"FB", "E"}, {"GB", "F#"}, {"AB", "G#"}, {"BB", "A#"}, {"CB", "B"}, {NULL, NULL}};
...
static int encode_notation(const char *notation, int bpm, uint8_t out[BLUEJAY_ARRAY_SIZE]) {
  ...
  for (int d = 1; d <= 32; d++)
    if (dur_counts[d] > best_dur_cnt) {
      best_dur_cnt = dur_counts[d];
      default_dur = d;
    }

  int default_oct = 5, best_oct_cnt = 0;
  for (int o = 4; o <= 7; o++)
    if (oct_counts[o] > best_oct_cnt) {
      best_oct_cnt = oct_counts[o];
      default_oct = o;
    }
